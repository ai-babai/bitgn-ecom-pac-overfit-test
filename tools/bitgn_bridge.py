...
class DeterministicSolver:
    ...
    def call(self, tool: str, args: dict[str, Any]) -> Any:
        self.step += 1
        return protobuf_to_jsonable(self.gateway.call(step=self.step, tool=tool, args=args))
    ...
    def completion(self, message: str, outcome: str, refs: list[str], solver: str = "deterministic") -> dict[str, Any]:
        clean = []
        for ref in refs:
...
def autosolve(args: argparse.Namespace) -> int:
    ws = open_task_workspace(args.workspace)
    gateway = ToolGateway.from_workspace_context(ws)
    report = DeterministicSolver(gateway, args.instruction).solve()
    emit({"ok": True, **report})
    return 0
...
def run_task(args: argparse.Namespace) -> int:
    ...
    if args.env.startswith("pac1"):
        ...
        report = DeterministicSolver(gateway, trial.instruction).solve()
